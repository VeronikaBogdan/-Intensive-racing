...
function getQuantityElenents(heightElement) {
...
function startGame() {
    ...
    for (let i = 0; i < getQuantityElenents(100); i++){
    ...
    for (let i = 0; i < getQuantityElenents(100 * setting.traffic); i++){
        const enemy = document.createElement('div');
        enemy.classList.add('enemy');
        enemy.y = -100 * setting.traffic * (i + 1);
        enemy.style.left = Math.floor(Math.random() * (gameArea.offsetWidth - 50)) + 'px';
        enemy.style.top = enemy.y + 'px';
        enemy.style.background = 'transparent url("./image/my_yellow_car.png") center / cover no-repeat';
